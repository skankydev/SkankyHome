...
abstract class FormBuilder {
    ...
    protected array $submitAttributes = [];
    ...
    public function __construct(array $link = [], string $method = 'POST', array $attributes = []) {
        ...
        // Charger les types de champs depuis la config
        ...
        // Appeler la méthode build() définie dans les classes enfants
        $this->build();
    }
    ...
    public function submit(string $label, array $attributes = []): self {
        ...
        $this->submitAttributes = $attributes;
        return $this;
    }
    ...
    protected function renderSubmit(): string {
        ...
        foreach ($this->submitAttributes as $key => $value) {
            $html .= ' ' . $key . '="' . htmlspecialchars($value) . '"';
        }
        
        $html .= '>' . htmlspecialchars($this->submitLabel) . '</button>';
        $html .= '</div>';
